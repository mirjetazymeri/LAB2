...
const updatePost = async(req, res) => { 
    try{ 
        if(req.file !== undefined) {
            ...
            await post.findByIdAndUpdate({_id:id}, {$set: {title: title, description: description,
                price: price, image:filename, countInStock:countInStock, brand:brand}});
            ...
            await Post.findByIdAndUpdate({_id:id}, {$set: {title: title, description: description,
                price: price, countInStock:countInStock, brand:brand}})
            res.send(200).send({success: true, msg:'Post updated successfully'});
        }
    }catch (error) {
        res.status(400).send({success:false, msg: error.message});
